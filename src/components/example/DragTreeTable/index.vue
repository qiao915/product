<template>
  <div class="drag-tree-table">
    ...
    <div class="drag-tree-table-body" draggable="false" @dragover="draging" @dragend="drop">
      <row depth="0" :columns="data.columns"
        :model="item" v-for="(item, index) in data.list" :key="index">
      </row>
    </div>
  </div>
</template>
...
export default {
  ...
  props: {
    data: Object,
    onDrag: Function,
    isDrag: Boolean // props 表示表格是否支持拖拽
  },
  data() {
    return {
      treeData: [],
      ...
      function pushData(curList, needPushList) {
        ...
        for (let i = 0; i < curList.length; i++) {
          ...
          if (_this.targetId == item.id) {
            const curDragItem = _this.getCurDragItem(_this.data.list, window.dragId)
          ...
          if (item.children && item.children.length) {
            pushData(item.children, obj.children)
          }
        }
      }
