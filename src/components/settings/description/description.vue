<template>
  <div class="description box">
    <breadcurmb :pathList="pathList"></breadcurmb>
    <title-info :info="$t('description.title-info')" :titleInfo="'description'"></title-info>
    <div class="size12 marTop10 marBott10 el-row is-align-middle el-row--flex">
       <el-button type="primary" native-type="button" size="small" class="colorF size12 add-module" @click="openNewModule">
                  {{$t('description.btn-text')}}
                </el-button>
    </div>
    <div class="table-box">
      <el-table
        :data="tableData"
        border
        size="small"
        header-row-class-name="size14"
        style="width: 100%">
          <el-table-column
            prop="name"
            :label='$t("description.table-name-text")'
            min-width="20%"
            show-overflow-tooltip>
          </el-table-column>
          <el-table-column
            prop="aboutClass"
            :label="$t('description.table-about-text')"
            show-overflow-tooltip>
            <template slot-scope="scope">
              {{aboutClassChange(scope.row.aboutClass)}}
            </template>
          </el-table-column>
          <el-table-column
            prop="createTime"
            :label='$t("description.table-time-text")'
            min-width="15%"
            show-overflow-tooltip>
          </el-table-column>
          <el-table-column
            prop="state"
            :label="$t('description.table-state-text')"
            min-width="15%"
            >
            <template slot-scope="scope">
              <el-switch
                class="switch-btn"
                v-model="scope.row.state"
                active-color="#25AC42"
                inactive-color="#BBBBBB"
                @change ='stateChange(scope.row.id)'>
              </el-switch>
              <i class="el-icon-edit-outline pointer size18 edit-btn" :title="$t('button.edit')" v-if="scope.row.state" @click="editDesc(scope.row)"></i>
               <i class="el-icon-delete pointer size18 edit-btn" :title="$t('button.edit')" v-else @click="editDesc(scope.row)"></i>
            </template>
          </el-table-column>
        </el-table>
    </div>
    <!-- 分页 -->
    <div class="page-box">
      <el-pagination
        layout="total, sizes, prev, pager, next, jumper"
        :pager-count= 5
        :page-sizes="[10, 20, 30, 40 ,50]"
        :page-size="pageSize"
        :total="total"
        @size-change="handleSizeChange"
        @current-change="handleCurrentChange">
      </el-pagination>
    </div>
    <alert-description ref="alertDescription"></alert-description>
  </div>
</template>

<script type="es6">
import Breadcurmb from '../../commonModule/breadcrumb'
import TitleInfo from '../../commonModule/titleInfo'
import AlertDescription from './alertDescription'
  export default {
    data() {
        return {
          pathList:[{"title":this.$t('description.title-parent'),"path":""},{"title":this.$t('description.title-next'),"path":""}],
          tableData : [
            {
              id: '1',
              name: '退货政策',
              aboutClass: ['衣服','汽车配件'],
              content: '123456',
              createTime:'2018-10-11 23:11',
              state: false,
            },
            {
              id: '1',
              name: '退货政策',
              aboutClass: ['衣服','汽车配件'],
              content: 'aaaaaaa',
              createTime:'2018-10-11 23:11',
              state: false,
            },
            {
              id: '1',
              name: '退货政策',
              aboutClass: ['衣服','汽车配件'],
              content: 'cccccccccccc',
              createTime:'2018-10-11 23:11',
              state: false,
            }
          ],
          pageNumber: 1,
          pageSize: 10,
          total: 0,
        }
    },
    created() {
      console.log('aaa')
    },
    mounted() {

    },
    methods: {
      openNewModule() {
        this.$refs.alertDescription.openAlert()
      },//编辑按钮
      editDesc(data) {
        console.log(data)
        this.$refs.alertDescription.openAlert(data)
      },
      //品类处理
      aboutClassChange(ary){
        let all =''
        ary.forEach((item)=> {
          all = all+ item+' ; '
        })
        all = all.substring(0,all.length-2)
        return all
      }
      ,//状态更改
      stateChange(id) {
        console.log(id)
      }, //分页 pageSize
      handleSizeChange(val) {
        console.log('pageSize'+val)
      },// 分页pageNumber
      handleCurrentChange(val){
        console.log('pageNumber'+val)
      }

    },
    components: {
      Breadcurmb,
      TitleInfo,
      AlertDescription
    },
  }
</script>

<style scoped>
  .add-module{
    margin-left: auto;
  }
  .table-box {
    text-align: center;
  }
  .table-box /deep/ .el-table th{
    text-align: center;
  }
  .table-box .switch-btn{
    vertical-align: middle
  }
  .table-box .edit-btn{
    vertical-align: middle;
    margin-left:20%;
  }
  .table-box /deep/ .el-table__body tr.current-row > td{
    background: #fff;
  }
  .page-box {
    text-align: right;
    padding-right:20px;
    margin-top: 20px;
  }
</style>
